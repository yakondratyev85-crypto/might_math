    ...
    <section className="screen">
      ...
      <div className="chapter-road">
        {chapters.map((chapter) => {
          ...
          return (
            <GlassCard asButton className={`chapter-card ${!unlocked ? 'is-locked' : ''}`} key={chapter.id} onClick={() => onPickChapter(chapter)}>
              ...
              <div className="stars-row" aria-label="Звезды"><IconBadge icon="ui_star" size="xs" /> {stats.stars}/30 {unlocked ? '' : '· закрыта'}</div>
            </GlassCard>
          );
        })}
      </div>
